<template>
    <div>
        <div class="flex justify-center">
            <div class="bg-base-200 cursor-pointer m-5 border-2 border-gray-300 h-[330px] w-[330px] rounded-md ">
                <div class=" h-[200px] w-[250px] rounded-md overflow-hidden m-10 ">
                    <img :src="useImage.previewImage || useArtist.artistEdit.url_image"
        ...
        <div class="flex justify-around">
            <div @click="$emit('actionArtist', inputName)" :class="[loading ? 'loading' : '']" class="btn-primary btn btn-outline">
                sua
            </div>
...
let One = 0;
onUpdated(() => {
    if(useArtist.artistEdit.name && One =='0'){
        One = 1;
        inputName.value = useArtist.artistEdit.name;
    }
})
</script>
